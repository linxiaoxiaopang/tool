...
export const Control = class Control {
  ...
  tl(icon) {
    ...
    const { moveL, moveT } = getTlMoveLAndMoveTByActiveObject.call(this, activeObject)
    objectControls.tl = new fabric.Control({
      x: -0.5,
      y: -0.5,
      sizeX: ICON_SIZE,
    ...
    function getTlMoveLAndMoveTByActiveObject(activeObject) {
      ...
      if (angle > 90 && x > this.designWidth) {
        moveL = (x - this.designWidth) / cos((PI / 180) * -angle) + HALF_ICON_SIZE / cos((PI / 180) * -angle)
      }

      if (angle > 90 && y > this.designHeight) {
        moveT = (y - this.designHeight) / cos((PI / 180) * -angle) + HALF_ICON_SIZE / cos((PI / 180) * -angle)
      }
      return {
        moveL: -moveL,
        moveT: -moveT
  ...
  br(icon) {
    const activeObject = this.canvas.getActiveObject()
    const { moveL, moveT } = getBrMoveLAndMoveTByActiveObject.call(this, activeObject)
    objectControls.br = new fabric.Control({
      x: 0.5,
      y: 0.5,
      sizeX: ICON_SIZE,
    ...
    function getBrMoveLAndMoveTByActiveObject(activeObject) {
      ...
      if (angle) {
        ...
        posRight = x
        posBottom = y
        if (posRight > this.designWidth) {
          moveL = -(
            ((x - this.designWidth) / cos((PI / 180) * -angle)) * character +
            (HALF_ICON_SIZE / cos((PI / 180) * -angle)) * character
          )
        }
        if (posBottom > this.designHeight) {
          moveT = -(
            ((y - this.designHeight) / cos((PI / 180) * -angle)) * character +
            (HALF_ICON_SIZE / cos((PI / 180) * -angle)) * character
          )
        ...
        moveL = Math.min(this.designWidth - posRight, 0)
        moveT = Math.min(this.designHeight - posBottom, 0)
      }
  ...
  mtr(icon) {
    const activeObject = this.canvas.getActiveObject()
    const { moveL, moveT } = getMtrMoveLAndMoveTByActiveObject.call(this, activeObject)
    objectControls.mtr = new fabric.Control({
      x: 0,
      y: -0.5,
      actionHandler: controlsUtils.rotationWithSnapping,
    ...
    function getMtrMoveLAndMoveTByActiveObject(activeObject) {
      ...
      if (x > this.designWidth - 40 - HALF_ICON_SIZE) {
        if (x < this.designWidth) {
          moveT = 10 * (sin((PI / 180) * -angle) || 1)
        } else {
          moveT = (x - this.designWidth + ICON_SIZE) * (sin((PI / 180) * -angle) || 1)
          moveL = x - this.designWidth + ICON_SIZE
        }
      }

      if (y > this.designHeight - 40 - HALF_ICON_SIZE) {
        if (y < this.designHeight) {
          moveT = 10 / cos((PI / 180) * -angle)
        } else if (angle < 90) {
          moveT = (y - this.designHeight) / sin((PI / 180) * -angle)
        } else {
          moveT = (y - this.designHeight) / cos((PI / 180) * -angle)
        }
      }
      console.log('moveT')
