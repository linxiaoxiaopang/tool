/* eslint-disable */
import { DESIGN_AREA_H, DESIGN_AREA_W } from '@/utils/constant'
...
export const Control = class Control {
  ...
  tl(icon) {
    ...
    function getTlMoveLAndMoveTByActiveObject(activeObject) {
      ...
      if (angle > 90 && x > DESIGN_AREA_W) {
        moveL = (x - DESIGN_AREA_W) / cos((PI / 180) * -angle) + HALF_ICON_SIZE / cos((PI / 180) * -angle)
      }

      if (angle > 90 && y > DESIGN_AREA_H) {
        moveT = (y - DESIGN_AREA_H) / cos((PI / 180) * -angle) + HALF_ICON_SIZE / cos((PI / 180) * -angle)
      }
      return {
        moveL: -moveL,
        moveT: -moveT
  ...
  br(icon) {
    ...
    function getBrMoveLAndMoveTByActiveObject(activeObject) {
      ...
      if (angle) {
        ...
        posRight = x
        posBottom = y
        if (posRight > DESIGN_AREA_W) {
          moveL = -(
            ((x - DESIGN_AREA_W) / cos((PI / 180) * -angle)) * character +
            (HALF_ICON_SIZE / cos((PI / 180) * -angle)) * character
          )
        }
        if (posBottom > DESIGN_AREA_H) {
          moveT = -(
            ((y - DESIGN_AREA_H) / cos((PI / 180) * -angle)) * character +
            (HALF_ICON_SIZE / cos((PI / 180) * -angle)) * character
          )
        ...
        moveL = Math.min(DESIGN_AREA_W - posRight, 0)
        moveT = Math.min(DESIGN_AREA_H - posBottom, 0)
      }
    ...
    function getMtrMoveLAndMoveTByActiveObject(activeObject) {
      ...
      if (x - 40 - HALF_ICON_SIZE < 0) {
        if (x > 0) {
          ...
          moveT = (x - 40 + ICON_SIZE) * (sin((PI / 180) * -angle) || 1)
          moveL = x - ICON_SIZE
        }
      }

      if (x > DESIGN_AREA_W - 40 - HALF_ICON_SIZE) {
        if (x < DESIGN_AREA_W) {
          moveT = 10 * (sin((PI / 180) * -angle) || 1)
        } else {
          moveT = (x - DESIGN_AREA_W + ICON_SIZE) * (sin((PI / 180) * -angle) || 1)
          moveL = x - DESIGN_AREA_W + ICON_SIZE
        }
      }
      ...
      if (y > DESIGN_AREA_H - 40 - HALF_ICON_SIZE) {
        if (y < DESIGN_AREA_H) {
          moveT = 10 / cos((PI / 180) * -angle)
        } else if (angle < 90) {
          moveT = (y - DESIGN_AREA_H) / sin((PI / 180) * -angle)
        } else {
          moveT = (y - DESIGN_AREA_H) / cos((PI / 180) * -angle)
        }
      }

      if (x >= 0 && x <= DESIGN_AREA_W - ICON_SIZE && y >= 0 && y <= DESIGN_AREA_H - ICON_SIZE) {
        moveT = -40
        moveL = 0
      }
      ...
      if (angle == 0) {
        if (x < 0) {
          ...
          if (y >= 40 + ICON_SIZE) {
            moveT = -40
          } else if (y < 40 + ICON_SIZE && y >= 0) {
            moveT = y
          } else if (y < 0) {
            moveT = y
          }
        } else if (x >= 0 && x <= DESIGN_AREA_W - ICON_SIZE) {
          if (y < 40 + ICON_SIZE && y >= 0) {
            moveT = -y + ICON_SIZE / 2
          } else if (y < 0) {
            moveT = -y + 40
          }
        } else if (x >= DESIGN_AREA_W - ICON_SIZE) {
          moveT = -40
          moveL = -(x - (DESIGN_AREA_W - ICON_SIZE))
        }
      } else if (angle <= 45) {
        if (x < 0) {
          moveL = (-x + ICON_SIZE / 2) / cos((PI / 180) * angle)
          if (y >= 40 + ICON_SIZE) {
            moveT = -40
          } else if (y < 40 + ICON_SIZE && y >= 0) {
            moveT = y
          ...
          moveT = -40 * sin((PI / 180) * angle)
          moveL = -(x - (DESIGN_AREA_W - ICON_SIZE)) / cos((PI / 180) * angle)
        }
      } else if (angle <= 90) {
        if (x < 0) {
          moveL = (-x + ICON_SIZE / 2) / cos((PI / 180) * angle)
        ...
        } else if (x >= 0 && x < DESIGN_AREA_W - (40 + ICON_SIZE) * sin(degreesToRadiansAngle)) {
          if (y < 40 + ICON_SIZE && y >= 0) {
            moveT = y + ICON_SIZE / 2
          } else if (y < 0) {
            moveT = -y + 40
          }
        } else if (x >= DESIGN_AREA_W - (40 + ICON_SIZE) * sin(degreesToRadiansAngle) && x < DESIGN_AREA_W) {
          // moveT = 40 + (x - DESIGN_AREA_W - (ICON_SIZE - 40)) * sin((PI / 180) * angle)
          moveT = -(DESIGN_AREA_W - x - ICON_SIZE / 2) * sin(degreesToRadiansAngle)
          moveL = 0
          // moveL = -(x - (DESIGN_AREA_W - ICON_SIZE)) / cos((PI / 180) * angle)
        } else if (x >= DESIGN_AREA_W) {
          moveT = 40 - (DESIGN_AREA_W - x) * sin(degreesToRadiansAngle)
          moveL = 0
        }
      } else if (angle <= 135) {
        if (x < 0) {
      ...
      console.log('(ICON_SIZE / 2 - 40) / cos((PI / 180) * angle)', (ICON_SIZE / 2 + 40) * cos(degreesToRadiansAngle))
      console.log('x', x)
      console.log('y', y)
