...
export default {
  ...
  methods: {
    ...
    mouseMove(x, y) {
      !this.disabledReactive && this.update()
      const { mouse } = this
      mouse.x = x / this.cssScale
      mouse.y = y / this.cssScale
    },
    mouseLeave() {
      this.mouseEnterFlag = false
    },
    update: function update() {
      const rect = getBoundingClientRect(this.$zoomRegion)
      //css进行缩放之后 宽度和高度也要进行缩放
      const keys = ['width', 'height']
      keys.map(key => rect[key] = rect[key] / this.cssScale)
      Object.assign(this.zoomRegionRect, rect)
    }
  }
}
</script>
