<template>
  <div :class="['tableForm', topPosition && 'topPosition', customClass]">
    <el-form
      ...
      :size="size"
      :showMessage="!$attrs.disabled"
      v-bind="$attrs"
      @submit.native.prevent
    >
      <el-button :loading="loading" class="form-del-btn" v-if="showDelBtn" @click="delFormHandler"> 删除 </el-button>
      <el-row :gutter="gutter">
        <el-col
          :span="colNum"
          v-for="(
            { label, prop, name, readonly, placeholder, isNumber, type, rule, colNum, slotName, hidden }, indx
          ) in cols"
          :key="indx"
          v-if="!hidden"
        >
          ...
          <el-form-item label="" :prop="prop" :rules="rule" v-else-if="hidden"> </el-form-item>
