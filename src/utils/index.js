...
import { PICBIGSIZE, COMPRESS_MAX_NUM, MENU_TYPE_MENU } from '@/utils/constant'
...
//val 要传入的字符串  key 代表高亮显示关键字
export function brightenKeyword(val, key) {
  const keyWord = key
  val = val.replace(/<\/?.+?>/g, '')
  key = key.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, '\\$&')
  const reg = new RegExp(key, 'g')
  return val?.replace(reg, `<span style="color:#3841DB">${keyWord}</span>`)
}
...
export function createUUidName(suffix = 'png') {
  return `${getUUID().replace(/-/g, '')}.${suffix}`
}
    ...
    if (file.type == 'image/jpeg' && file.size >= PICBIGSIZE) {
      ...
      reader.onload = function (e) {
        ...
        if (start && end) {
          ...
          resolve({
            url: typeof callback === 'function' ? callback(URL.createObjectURL(file)) : URL.createObjectURL(file)
          })
        }
      }
  ...
  return new Promise((resolve) => {
    ...
    img.onerror = () => {
      if (!lock) {
        lock = true
        if (src.length < 200) {
          const suffix = (src || '').indexOf('?') >= 0 ? '&uid' + createRandomNum() : '?uid' + createRandomNum()
          img.src = src + suffix
        } else {
          img.src = src
        }
    ...
    return results.map((item) => {
      ...
      if (item.children) {
        for (let i = 0; i < item.children.length; i++) {
          ...
          if (sonItem.menuType !== MENU_TYPE_MENU) {
            sonItem.hidden = true
...
export const isUatOrPro =
  process.env.VUE_APP_BASE_URL === 'www.uat.zdcustom.com' || process.env.VUE_APP_BASE_URL === 'www.zdcustom.com'
